  ...
  <div class="banner-wrap movie-reviews">
    <div class="banner grid-limit">
      ...
      <p class="banner-sections">
        ...
        <span class="banner-section">Haber & Duyurular</span>
      </p>
    </div>
  </div>
...
$toplam_icerik = $db->prepare("select count(*) from makaleler where (yayin = '1') ");
...
$haberler = $db->query("select * from makaleler where (yayin = '1') order by stamp desc LIMIT ".$limit." , ".$sayfada."");
...
$metin = mb_substr(strip_tags($haber['icerik']),0,'150','utf-8').'...' ;
	echo '<div class="post-preview medium movie-news">';
          echo '<a href="'.$domain2.'haber/'.$haber['urlkodu'].'">';
            ...
            echo '</div>';
          echo '</a>';
          echo '<a href="'.$domain.'kategori/'.$kategori['kategorikodu'].'" style="background-color:'.$kategori['renk'].';" class="tag-ornament">'.$kategori['kategori_TR'].'</a>';
...
if($sayfa != 1){
?>	
<a href="<?php echo $domain2.'haberler'; ?>?sayfa=<?php echo $sayfa-1; ?>" aria-controls="bootstrap-data-table" data-dt-idx="0" tabindex="0" class="page-link">
...
for($s = $sol_sayfalar; $s <= $sag_sayfalar; $s++) {
					...
					?>
						<a class="page-navigation-item" href="<?php echo $domain2.'haberler'; ?>?sayfa=<?php echo $s; ?>" aria-controls="bootstrap-data-table" data-dt-idx="1" tabindex="0" class="page-link"><?php echo $s; ?></a>
					<?php
					}
				}
