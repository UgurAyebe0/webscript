...
$okunduyap = $db->query("update uyemesajlar set okunma = '1' where (mesajid = '".$_REQUEST['p']."') && (gonderenkisi != '".$_SESSION['username']."')");
$konuu = $db->prepare("select * from uyemesajlar where (mesajid = '".$_REQUEST['p']."') order by stamp asc limit 1");
$result = $konuu->execute();
$konuu   = $konuu->fetch(PDO::FETCH_ASSOC);
?>
...
<a href="<?php echo $domain2.'gonderilen'; ?>"><p class="option-list-item">Giden Kutusu</p></a>

<a href="<?php echo $domain2.'mesajyaz'; ?>"><p class="option-list-item">Mesaj Yaz</p></a>
<a href="<?php echo $domain2.'detay/'.$_REQUEST['p']; ?>"><p class="option-list-item selected" style="text-transform:none;"><?php echo $konuu['konu']; ?></p></a>
</div>
</div>
<div class="col-md-12" style="margin-top:15px;"> 
<?php
$mesajlar = $db->query("select * from uyemesajlar where (mesajid = '".$_REQUEST['p']."') order by stamp asc");
$mesajlar->execute();
...
echo '</div>';
echo '<div class="table-row-header-item padded-big" style="display:table-cell;">';
  echo '<p class="table-row-header-title">Mesaj</p>';
echo '</div>';
echo '<div class="table-row-header-item padded-big" style="display:table-cell;">';
...
$mesajyazannnn = $kpanel->prepare("select * from Kullanicilar where (username = '".$mes['gonderenkisi']."')");
...
echo '<div class="team-info-wrap">';
		  echo '<img class="team-logo small" src="https://cravatar.eu/avatar/'.$mes['gonderenkisi'].'" alt="'.$mes['gonderenkisi'].'">';
		  echo '<div class="team-info">';
			echo '<a href="'.$domain.'profil/'.$mes['gonderenkisi'].'" style="color:'.$renkkkk.' !important;"><p class="team-country" style="font-size: .8em;color:'.$renkkkk.' !important;text-transform:none;">'.$mes['gonderenkisi'].' <span class="tag-ornament" style="vertical-align:middle;margin-top: 0px;margin-left: 5px;background:'.$renkkkk.';">'.$grupadiiii['name'].'</span></a></p>';
		  echo '</div>';
		echo '</div>';
...
echo '<p class="table-text bold" style="text-transform:none;">'.$mes['mesaj'].'</p>'; 
echo '</div>'; 
echo '<div class="table-row-item" style="display:table-cell;min-width:130px;padding-left:10px;">'; 
echo '<p class="table-text bold" style="text-align:left;">'.date("d.m.Y H:i",$mes['stamp']).'</p>'; 
echo '</div>'; 
echo '</div>';
...
<div class="filters-row">
<div class="option-list">
<p class="option-list-item selected">Cevapla</p>
...
<form class="account-settings-form" method="POST" style="margin-top:15px;">
	...
	<div class="form-row">
	  <div class="form-item blue">
		<textarea type="text" id="mesaj" placeholder="Yanıtınız" name="mesaj" rows="5"  required ></textarea>
...
<?php				
if($_POST['mesaj'] != ''){
$alicikisii = $db->prepare("select * from uyemesajlar where (mesajid = '".$_REQUEST['p']."') limit 1");
$result = $alicikisii->execute();
$alicikisii   = $alicikisii->fetch(PDO::FETCH_ASSOC);
if($alicikisii['alankisi'] != $_SESSION['username']){
	$alankisi = $alicikisii['alankisi'];
}else{
	$alankisi = $alicikisii['gonderenkisi'];
}
			$mesajid = $_REQUEST['p'];
			$gonderenkisi = $_SESSION['username'];
			$konu = $konuu['konu'];
			$mesaj = $_POST['mesaj'];
			$stamp = time();
			$okunma = '0';
			...
			$kaydet = $db->query("insert into uyemesajlar (mesajid, alankisi, gonderenkisi, konu, mesaj, stamp, okunma, yayin) values ('$mesajid', '$alankisi', '$gonderenkisi', '$konu', '$mesaj', '$stamp', '$okunma', '$yayin') ");
			$_SESSION['gonderildi'] = 'Mesajınız başarıyla gönderilmiştir.';
			header('Location:'.$domain2.'detay/'.$_REQUEST['p']);	
