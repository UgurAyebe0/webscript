...
<a href="<?php echo $domain2.'mesajlarim'; ?>"><p class="option-list-item selected">Mesajlarım</p></a>
...
<?php
$mesajlar = $db->query("select * from uyemesajlar where (alankisi = '".$_SESSION['username']."') group by mesajid order by stamp desc");
$mesajlar->execute();
...
echo '<div class="table-rows">';
	...
	foreach($mesajlar as $mes){
		...
		$okunmakont = $db->prepare("select count(*) from uyemesajlar where (mesajid = :mesajid) && (okunma = '0') && (gonderenkisi != '".$kredi['username']."') group by mesajid");
		$result = $okunmakont->execute(array(":mesajid" => $mesajid));
		$okunmakont  = $okunmakont->fetchColumn();	
	if($okunmakont > 0){
		$cl = '<br><br> <span class="text-success"><i class="fa fa-asterisk fa-spin"></i> Yeni Mesaj Var  </span>';
	}else{
		$cl = '';
	}	
...
echo '<div class="team-info-wrap">';
		  echo '<img class="team-logo small gizle" src="https://cravatar.eu/avatar/'.$mes['gonderenkisi'].'" alt="'.$mes['gonderenkisi'].'">';
		  echo '<div class="team-info">';
			echo '<a href="'.$domain2.'detay/'.$mesajid.'" style="color:'.$renkkkk.' !important;"><p class="team-country" style="font-size: .8em;color:'.$renkkkk.' !important;text-transform:none;">'.$mes['gonderenkisi'].' <span class="tag-ornament gizle" style="vertical-align:middle;margin-top: 0px;margin-left: 5px;background:'.$renkkkk.';">'.$grupadiiii['name'].'</span></a></p>';
		  echo '</div>';
		echo '</div>';
...
echo '<div class="table-row-item" style="display:table-cell;">'; 
echo '<a href="'.$domain2.'detay/'.$mesajid.'"><p class="table-text bold">+'.$mesajsayisi.' '.$cl.'</p></a>'; 
echo '</div>'; 
