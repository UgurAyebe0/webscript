...
<a href="<?php echo $domain2.'mesajyaz'; ?>"><p class="option-list-item selected">Mesaj Yaz</p></a>
</div>
</div>
...
.ui-autocomplete-loading { background:url('<?php echo $domain; ?>yukleniyor.gif') no-repeat 95% !important;background-size:auto 40px !important; }
</style>

<form class="account-settings-form" method="POST" style="margin-top:15px;">
	<div class="form-row">
	  <div class="form-item blue">
		<label for="alici" class="rl-label">Alıcı</label>
		<input id="alici" class="ui-autocomplete-input" type="text" placeholder="Kullanıcı Seç" autocomplete="off"  name="alici" value="<?php echo $_POST['kullanici']; ?>" required>
   
	  </div> 
	</div>
	<div class="form-row">
	  <div class="form-item blue">
		<label for="konu" class="rl-label">Konu</label>
		<input type="text" id="konu" placeholder="Konu" name="konu" required >
	...
	<div class="form-row">
	  <div class="form-item blue">
		<label for="mesaj" class="rl-label">Mesaj</label>
		<textarea type="text" id="mesaj" placeholder="Mesajınız" name="mesaj" rows="5"  required ></textarea>
...
<?php
if($_POST['alici'] != ''){
	$alicikontrol = $kpanel->prepare("select * from Kullanicilar where (username = '".$_POST['alici']."')");
	$result = $alicikontrol->execute();
	$alicikontrol  = $alicikontrol->fetch(PDO::FETCH_ASSOC);
	if($alicikontrol['username'] != NULL){
			$mesajid = uniqid();
			$alankisi = $_POST['alici'];
			...
			header('Location:'.$domain2.'mesajyaz');	
	}else{
