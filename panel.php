...
<a href="<?php echo $domain2.'panel'; ?>"><p class="option-list-item selected">Forum Mesajlarım</p></a>
<a href="<?php echo $domain2.'videoyorumlarim'; ?>"><p class="option-list-item">Video Yorumlarım</p></a>
<a href="<?php echo $domain2.'haberyorumlarim'; ?>"><p class="option-list-item">Haber Yorumlarım</p></a>
</div>
</div>
<?php
$sayfada = 20;

$toplam_icerik = $db->prepare("select count(*) from forummesajlar where (username = '".$_SESSION['username']."')");
...
$sonforum = $db->query("select * from forummesajlar where (username = '".$_SESSION['username']."') order by stamp desc LIMIT ".$limit." , ".$sayfada."");
...
<div class="table-row-header-item padded-big" style="display:table-cell;">
	<p class="table-row-header-title">Kategori</p>
</div>
<div class="table-row-header-item padded-big" style="display:table-cell;">
...
<?php
	...
	$i = 0;
	foreach($sonforum as $son){
		$secilikonu = $db->prepare("select * from forumkonular where (id = '".$son['konuid']."')");
		$result = $secilikonu->execute();
		$secilikonu = $secilikonu->fetch(PDO::FETCH_ASSOC);

		
		$secilikategori = $db->prepare("select * from forumkategoriler where (id = '".$secilikonu['forumkategori']."')");
		$result = $secilikategori->execute();
		$secilikategori = $secilikategori->fetch(PDO::FETCH_ASSOC);
		if(!empty($secilikonu) or !empty($secilikategori)){
		$i++;
		...
		echo '</div>';
		echo '<div class="table-row-item" style="display:table-cell;">';
			echo '<a href="'.$domain2.'forumkategori/'.$secilikategori['kategorikodu'].'"><p class="table-text">'.$secilikategori['kategori_TR'].'</p></a>';
		echo '</div>';
		echo '<div class="table-row-item" style="display:table-cell;">';
			echo '<a href="'.$domain2.'forumdetay/'.$secilikategori['id'].'/'.$secilikonu['id'].'" style="text-decoration:none;"><p class="table-text">'.$secilikonu['baslik'].'</p></a>';
		echo '</div>';
		echo '<div class="table-row-item" style="display:table-cell;">';
		echo '<p class="table-text">'.mb_substr(strip_tags($son['mesaj']),0,25,'UTF-8').'...</p>';
		echo '</div>';
...
<?php
	...
	echo '<p class="text-center light">Henüz mesajınız bulunmamaktadır.</p>';
}
?>
</div>
