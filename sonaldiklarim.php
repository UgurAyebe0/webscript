...
$toplam_sayfa = ceil($alinanurun / $sayfada);
...
$alinanurunler = $kpanel->query("select * from UrunLog where (KullaniciId = '".$_SESSION["kid"]."') order by UrunLogId desc LIMIT ".$limit." , ".$sayfada."");
?>
  <!-- BANNER WRAP -->
...
?>
    ...
    <div class="layout-body">
      ...
      <div class="section-title-wrap blue">
        <h2 class="section-title medium">Geçmiş - Son Aldıklarım</h2>
...
<a href="<?php echo $domain2.'gecmis'; ?>"><p class="option-list-item">Son Kredi Yüklemelerim</p></a>
</div>
<div class="option-list">
<a href="<?php echo $domain2.'sonaldiklarim'; ?>"><p class="option-list-item selected">Son Aldığım Ürünler</p></a>
</div>
</div>
<?php
$i = '0';
...
<div class="table-row-header-item padded-big" style="display:table-cell;">
	<p class="table-row-header-title">Ödenen Kredi</p>
</div>
<div class="table-row-header-item padded-big" style="display:table-cell;">
...
$urun  = $urun->fetch(PDO::FETCH_ASSOC);	
		...
		echo '</div>';
		echo '<div class="table-row-item" style="display:table-cell;">';
			echo '<p class="table-text">'.$alinan['Tarih'].'</p>';
		echo '</div>';
		echo '<div class="table-row-item" style="display:table-cell;">';
			echo '<p class="table-text">'.$urun['Adi'].'</p>';
		echo '</div>';
		echo '<div class="table-row-item" style="display:table-cell;">';
			echo '<p class="table-text">'.$alinan['UrunFiyati'].'</p>';
		echo '</div>';
		echo '<div class="table-row-item" style="display:table-cell;">';
			echo '<p class="table-text">'.$durum.'</p>';
		echo '</div>';
...
<?php
	...
	echo '<p class="text-center light">Henüz ürününüz bulunmamaktadır.</p>';
}
?>
...
if ($toplam_sayfa>1) {
	if($sayfa != 1){
	?>
<a href="<?php echo $domain2.'sonaldiklarim'; ?>?sayfa=<?php echo $sayfa-1; ?>" aria-controls="bootstrap-data-table" data-dt-idx="0" tabindex="0" class="page-link">
...
for($s = $sol_sayfalar; $s <= $sag_sayfalar; $s++) {
					...
					?>
						<a class="page-navigation-item" href="<?php echo $domain2.'sonaldiklarim'; ?>?sayfa=<?php echo $s; ?>" aria-controls="bootstrap-data-table" data-dt-idx="1" tabindex="0" class="page-link"><?php echo $s; ?></a>
					<?php
					}
				}
