...
<a href="<?php echo $domain2.'videoyorumlarim'; ?>"><p class="option-list-item selected">Video Yorumlarım</p></a>
...
$toplam_icerik = $db->prepare("select count(*) from videoyorum where (username = '".$_SESSION['username']."')");
...
$sonyorum = $db->query("select * from videoyorum where (username = '".$_SESSION['username']."') order by stamp desc LIMIT ".$limit." , ".$sayfada."");
...
<?php
	...
	foreach($sonyorum as $son){
		$secilihaber = $db->prepare("select * from youtube where (id = '".$son['video']."')");
		...
		echo '</div>';
		echo '<div class="table-row-item" style="display:table-cell;">';
			echo '<a href="'.$domain2.'video/'.$secilihaber['urlkodu'].'" style="text-decoration:none;"><p class="table-text">'.$secilihaber['baslik'].'</p></a>';
		echo '</div>';
		echo '<div class="table-row-item" style="display:table-cell;">';
...
if($sayfa != 1){
?>	
<a href="<?php echo $domain2.'videoyorumlarim'; ?>?sayfa=<?php echo $sayfa-1; ?>" aria-controls="bootstrap-data-table" data-dt-idx="0" tabindex="0" class="page-link">
...
for($s = $sol_sayfalar; $s <= $sag_sayfalar; $s++) {
					...
					?>
						<a class="page-navigation-item" href="<?php echo $domain2.'videoyorumlarim'; ?>?sayfa=<?php echo $s; ?>" aria-controls="bootstrap-data-table" data-dt-idx="1" tabindex="0" class="page-link"><?php echo $s; ?></a>
					<?php
					}
				}
