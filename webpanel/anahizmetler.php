				...
				<div class="page-header page-header-default">
					<div class="page-header-content">
						<div class="page-title">
							<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">Anasayfa Bloklar <span> - Blok Listesi</h4>
						</div>
				...
				<div class="content">
					...
					<div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title"><i class="fa fa-th-list"></i> Anasayfa Blok Listesi</h5>
							<div class="heading-elements">
								<ul class="icons-list">
						...
						<table class="table datatable-basic">
							...
							<tbody>
<?php 
$sayfalar = $core->db->query("select * from anabloklar order by sira asc");
foreach($sayfalar as $sayfa){

		echo '<tr>';
			echo '<td>'.$sayfa['baslik'].'</td>';
			echo '<td>'.$sayfa['baslik_EN'].'</td>';
			echo '<td>'.$sayfa['link'].'</td>';
	...
	if($_POST[''.$sayfa['id'].'sil']){
		$core->db->query("delete from anabloklar where (id = :id);",array('id' => $sayfa['id']));
		header('Location:anahizmetler.php');
	}
	if($_POST[''.$sayfa['id'].'aktifpasif']){
		if($sayfa['yayin'] == '1'){
			$core->db->query("update anabloklar set yayin = '0' where (id = :id);",array('id' => $sayfa['id']));
		}else{
			$core->db->query("update anabloklar set yayin = '1' where (id = :id);",array('id' => $sayfa['id']));
		}
		header('Location:anahizmetler.php');
	}
	if($_POST[''.$sayfa['id'].'duzenle']){
		header('Location:anahizmetduzenle.php?id='.$sayfa['id'].'');
	}
}
?>
...
<script>
function sayfasil() {
return	confirm("Seçtiğiniz hizmet sistemden silinecektir, onaylıyor musunuz?");
}
</script>
</body>
</html>