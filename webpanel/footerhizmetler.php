				...
				<div class="page-header page-header-default">
					<div class="page-header-content">
						<div class="page-title">
							<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">Footer Menü <span> - Menü Listesi</h4>
						</div>
				...
				<div class="content">
					...
					<div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title"><i class="fa fa-th-list"></i> Menü Listesi</h5>
							<div class="heading-elements">
								<ul class="icons-list">
						...
						<table class="table datatable-basic">
							...
							<tbody>
<?php 
$sayfalar = $core->db->query("select * from footermenu order by stamp desc");
foreach($sayfalar as $sayfa){
		echo '<tr>';
			echo '<td>'.$sayfa['baslik'].'</td>';
			echo '<td>'.$sayfa['baslik_EN'].'</td>';
			echo '<td>'.$sayfa['sira'].'</td>';
	...
	if($_POST[''.$sayfa['id'].'sil']){
		$core->db->query("delete from footermenu where (id = :id);",array('id' => $sayfa['id']));
		header('Location:footerhizmetler.php');
	}
	if($_POST[''.$sayfa['id'].'aktifpasif']){
		if($sayfa['yayin'] == '1'){
			$core->db->query("update footermenu set yayin = '0' where (id = :id);",array('id' => $sayfa['id']));
		}else{
			$core->db->query("update footermenu set yayin = '1' where (id = :id);",array('id' => $sayfa['id']));
		}
		header('Location:footerhizmetler.php');
	}
	if($_POST[''.$sayfa['id'].'duzenle']){
		header('Location:footerhizmetduzenle.php?id='.$sayfa['id'].'');
	}
}
?>
