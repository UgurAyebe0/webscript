				...
				<div class="page-header page-header-default">
					<div class="page-header-content">
						<div class="page-title">
							<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">Projeler <span> - Proje Listesi</h4>
						</div>
				...
				<div class="content">
					...
					<div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title"><i class="fa fa-th-list"></i> Proje Listesi</h5>
							<div class="heading-elements">
								<ul class="icons-list">
						...
						<table class="table datatable-basic">
							...
							<tbody>
<?php 
$sayfalar = $core->db->query("select * from haberler order by stamp desc");
foreach($sayfalar as $sayfa){
		echo '<tr>';
			echo '<td><img src="../images/proje_resim/'.$sayfa['gorsel'].'" style="width:80px;"></td>';
	...
	if($_POST[''.$sayfa['id'].'sil']){
		$core->db->query("delete from haberler where (id = :id);",array('id' => $sayfa['id']));
		header('Location:haberler.php');
	}
	if($_POST[''.$sayfa['id'].'aktifpasif']){
		if($sayfa['yayin'] == '1'){
			$core->db->query("update haberler set yayin = '0' where (id = :id);",array('id' => $sayfa['id']));
		}else{
			$core->db->query("update haberler set yayin = '1' where (id = :id);",array('id' => $sayfa['id']));
		}
		header('Location:haberler.php');
	}
	if($_POST[''.$sayfa['id'].'duzenle']){
		header('Location:haberduzenle.php?id='.$sayfa['id'].'');
	}
}
?>
...
<script>
function sayfasil() {
return	confirm("Seçtiğiniz proje sistemden silinecektir, onaylıyor musunuz?");
}
</script>
</body>
</html>