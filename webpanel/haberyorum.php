				...
				<div class="page-header page-header-default">
					<div class="page-header-content">
						<div class="page-title">
							<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">Haberler <span> - Yorum Listesi</h4>
						</div>
				...
				<div class="content">
					...
					<div class="panel panel-flat">
						...
						<table class="table datatable-basic">
							...
							<tbody>
<?php 
$urunler = $db->query("select * from haberyorum order by stamp desc");
foreach($urunler as $urun){
	$secilihaber = $db->prepare("select * from makaleler where (id = '".$urun['haber']."')");
	$result = $secilihaber->execute();
	$secilihaber   = $secilihaber->fetch(PDO::FETCH_ASSOC);
	
	
		echo '<tr>';
			echo '<td>'.$secilihaber['baslik'].'</td>';
			echo '<td>'.$urun['username'].'</td>';
			echo '<td>'.$urun['yorum'].'</td>';
			echo '<td><span class="hidden">'.$urun['stamp'].'</span> '.date("d.m.Y H:i",$urun['stamp']).'</td>';
		if($urun['yayin'] == '1'){
	...
	if($_POST[''.$urun['id'].'sil']){
		$db->query("delete from haberyorum where (id = '".$urun['id']."')");
		header('Location:haberyorum.php');
	}
	if($_POST[''.$urun['id'].'aktifpasif']){
		if($urun['yayin'] == '1'){
			$db->query("update haberyorum set yayin = '0' where (id = '".$urun['id']."')");
		}else{
			$db->query("update haberyorum set yayin = '1' where (id = '".$urun['id']."')");
		}
		header('Location:haberyorum.php');
	}
