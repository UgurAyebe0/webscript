				...
				<div class="page-header page-header-default">
					<div class="page-header-content">
						<div class="page-title">
							<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">Hizmetler <span> - Hizmet Listesi</h4>
						</div>
					...
					<div class="breadcrumb-line">
						<ul class="breadcrumb">
							...
							<li class="active">Hizmet Listesi</li>
						</ul>
				...
				<div class="content">
					...
					<div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title"><i class="fa fa-th-list"></i> Hizmet Listesi</h5>
							<div class="heading-elements">
								<ul class="icons-list">
						...
						<table class="table datatable-basic">
							...
							<tbody>
<?php 
$sayfalar = $core->db->query("select * from hizmetler order by stamp desc");
foreach($sayfalar as $sayfa){
$secilikategori = $core->db->row("select * from yenihizmetkategori where (id = :id);",array('id' => $sayfa['kategori']));
		echo '<tr>';
	...
	if($_POST[''.$sayfa['id'].'sil']){
		$core->db->query("delete from hizmetler where (id = :id);",array('id' => $sayfa['id']));
		header('Location:hizmetler.php');
	}
	if($_POST[''.$sayfa['id'].'aktifpasif']){
		if($sayfa['yayin'] == '1'){
			$core->db->query("update hizmetler set yayin = '0' where (id = :id);",array('id' => $sayfa['id']));
		}else{
			$core->db->query("update hizmetler set yayin = '1' where (id = :id);",array('id' => $sayfa['id']));
		}
		header('Location:hizmetler.php');
	}
	if($_POST[''.$sayfa['id'].'duzenle']){
		header('Location:hizmetduzenle.php?id='.$sayfa['id'].'');
	}
}
?>
