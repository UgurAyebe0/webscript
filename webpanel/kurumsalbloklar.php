				...
				<div class="page-header page-header-default">
					<div class="page-header-content">
						<div class="page-title">
							<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">Kurumsal Bloklar <span> - Blok Listesi</h4>
						</div>
					...
					<div class="breadcrumb-line">
						<ul class="breadcrumb">
							...
							<li class="active">Blok Listesi</li>
						</ul>
				...
				<div class="content">
					...
					<div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title"><i class="fa fa-th-list"></i> Kurumsal Blok Listesi</h5>
							<div class="heading-elements">
								<ul class="icons-list">
						...
						<table class="table datatable-basic">
							...
							<tbody>
<?php 
$sayfalar = $core->db->query("select * from kurumsalbloklar order by sira asc");
foreach($sayfalar as $sayfa){

		echo '<tr>';
			echo '<td>'.$sayfa['baslik'].'</td>';
			echo '<td>'.$sayfa['baslik_EN'].'</td>';
	...
	if($_POST[''.$sayfa['id'].'sil']){
		$core->db->query("delete from kurumsalbloklar where (id = :id);",array('id' => $sayfa['id']));
		header('Location:kurumsalbloklar.php');
	}
	if($_POST[''.$sayfa['id'].'aktifpasif']){
		if($sayfa['yayin'] == '1'){
			$core->db->query("update kurumsalbloklar set yayin = '0' where (id = :id);",array('id' => $sayfa['id']));
		}else{
			$core->db->query("update kurumsalbloklar set yayin = '1' where (id = :id);",array('id' => $sayfa['id']));
		}
		header('Location:kurumsalbloklar.php');
	}
	if($_POST[''.$sayfa['id'].'duzenle']){
		header('Location:kurumsalblokduzenle.php?id='.$sayfa['id'].'');
	}
}
?>
...
<script>
function sayfasil() {
return	confirm("Seçtiğiniz blok sistemden silinecektir, onaylıyor musunuz?");
}
</script>
</body>
</html>