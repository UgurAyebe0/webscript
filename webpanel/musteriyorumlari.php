				...
				<div class="page-header page-header-default">
					<div class="page-header-content">
						<div class="page-title">
							<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">Müşteri Yorumları <span> - Yorum Listesi</h4>
						</div>
				...
				<div class="content">
					...
					<div class="panel panel-flat">
						...
						<table class="table datatable-basic">
							...
							<tbody>
<?php 
$sayfalar = $core->db->query("select * from yorumlar");
foreach($sayfalar as $sayfa){
		echo '<tr>';
			echo '<td><img src="../images/'.$sayfa['gorsel'].'" width="40" style="border-radius:4px;"></td>';
			echo '<td>'.$sayfa['adisoyadi'].'</td>';
	...
	if($_POST[''.$sayfa['id'].'sil']){
		$core->db->query("delete from yorumlar where (id = :id);",array('id' => $sayfa['id']));
		header('Location:musteriyorumlari.php');
	}
	if($_POST[''.$sayfa['id'].'aktifpasif']){
		if($sayfa['yayin'] == '1'){
			$core->db->query("update yorumlar set yayin = '0' where (id = :id);",array('id' => $sayfa['id']));
		}else{
			$core->db->query("update yorumlar set yayin = '1' where (id = :id);",array('id' => $sayfa['id']));
		}
		header('Location:musteriyorumlari.php');
	}
	if($_POST[''.$sayfa['id'].'duzenle']){
		header('Location:yorumduzenle.php?id='.$sayfa['id'].'');
	}
}
?>
