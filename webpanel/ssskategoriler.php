				...
				<div class="page-header page-header-default">
					<div class="page-header-content">
						<div class="page-title">
							<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">SSS Kategoriler <span> - Kategori Listesi</h4>
						</div>
					...
					<div class="breadcrumb-line">
						...
						<div class="" style="float:right;padding-top:5px;padding-bottom:5px;">
							...
							<a href="/webpanel/yenisss.php" class="btn btn-primary btn-sm" style="margin-right:10px;"> Yeni Soru <i class="fa fa-plus"></i></a>
							<a href="/webpanel/ssskategoriler.php" class="btn btn-warning btn-sm" style="margin-right:10px;"> Kategoriler <i class="fa fa-folder"></i></a>
				...
				<div class="content">
					...
					<div class="panel panel-flat">
						...
						<table class="table datatable-basic">
							...
							<tbody>
<?php 
$urunler = $db->query("select * from ssskategoriler");
foreach($urunler as $urun){
$urunsayisi = $db->prepare("select count(id) from sss where (kategori = '".$urun['id']."')");
...
$urunsayisi   = $urunsayisi->fetchColumn();
				...
				echo '<ul class="icons-list">';
					echo '<li class="dropdown">';
						...
						echo '<ul class="dropdown-menu dropdown-menu-right">';
							echo '<li><a href="/webpanel/ssskategoriduzenle.php?id='.$urun['id'].'" class="btn btn-block btn-sm btn-primary"  style="border-radius:0px;">Düzenle</a>';
	...
	if($_POST[''.$urun['id'].'sil']){
		$db->query("delete from ssskategoriler where (id = '".$urun['id']."')");
		header('Location:ssskategoriler.php');
	}
	if($_POST[''.$urun['id'].'aktifpasif']){
		if($urun['yayin'] == '1'){
			$db->query("update ssskategoriler set yayin = '0' where (id = '".$urun['id']."')");
		}else{
			$db->query("update ssskategoriler set yayin = '1' where (id = '".$urun['id']."')");
		}
		header('Location:ssskategoriler.php');
	}
	if($_POST[''.$urun['id'].'duzenle']){
		header('Location:ssskategoriduzenle.php?p='.$urun['id'].'');
	}
}
?>
...
<script>
function urunsil() {
return	confirm("Seçtiğiniz kategori sistemden silinecektir, eğer bu kategori altında soru varsa kategorisiz olacaktır, onaylıyor musunuz?");
}
</script>
</body>
</html>