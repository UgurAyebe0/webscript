				...
				<div class="page-header page-header-default">
					<div class="page-header-content">
						<div class="page-title">
							<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">Videolar <span> - Video Listesi</h4>
						</div>
				...
				<div class="content">
					...
					<div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title"><i class="fa fa-th-list"></i> Video Listesi</h5>
						...
						<table class="table table-asc0 datatable-basic">
							...
							<tbody>
<?php 
$sayfalar = $db->query("select * from youtube");
foreach($sayfalar as $sayfa){


		echo '<tr>';
			echo '<td><span class="hidden">'.$sayfa['stamp'].'</span>'.date("d.m.Y H:i",$sayfa['stamp']).'</td>';
			echo '<td><img src="../images/blog/'.$sayfa['kapak'].'" width="40" height="40" ></td>';
			echo '<td>'.$sayfa['baslik'].'</td>';
			echo '<td>'.$sayfa['link'].'</td>';
			echo '<td>'.$sayfa['yukleyen'].'</td>';
				...
				echo '<ul class="icons-list">';
					echo '<li class="dropdown">';
						...
						echo '<ul class="dropdown-menu dropdown-menu-right">';
							echo '<li><a href="/webpanel/videoduzenleme.php?id='.$sayfa['id'].'" class="btn btn-block btn-sm btn-primary"  style="border-radius:0px;">Düzenle</a>';
	...
	if($_POST[''.$sayfa['id'].'sil']){
		$db->query("delete from youtube where (id = '".$sayfa['id']."')");
		header('Location:videolar.php');
	}
	if($_POST[''.$sayfa['id'].'aktifpasif']){
		if($sayfa['yayin'] == '1'){
			$db->query("update youtube set yayin = '0' where (id = '".$sayfa['id']."')");
		}else{
			$db->query("update youtube set yayin = '1' where (id = '".$sayfa['id']."')");
		}
		header('Location:videolar.php');
	}
}
